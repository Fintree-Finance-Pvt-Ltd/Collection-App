...
async function getStatsForProduct(product, loanTable, { start, end }) {
  ...
  const repoQB = repossessionRepository
    .createQueryBuilder("r")
    .where("r.product = :partner", {
      partner: product,
    });
  ...
  const qbAgents = paymentRepository
    ...
    .leftJoin(User, "u", "u.id = p.collectedBy")
    .where("p.product = :product", { product });
...
router.get("/superAdmin/collections", authenticateToken, async (req, res) => {
  try {
    ...
    let partners;
    if (partnerQuery === "all") {
      const uniqueProducts = await paymentRepository
        .createQueryBuilder("p")
        .select("DISTINCT p.product", "product")
        .getRawMany();
      partners = uniqueProducts
    ...
    for (const p of partners) {
      ...
      if (districtsFilter.length) {
        where.push(`district IN (${districtsFilter.map(() => "?").join(",")})`);
    ...
    for (const p of partners) {
      ...
      const lanList = payRows.map((r) => r.lan).filter((x) => x);
      ...
      for (const r of payRows) {
        ...
        if (dealersFilter.length && !dealersFilter.includes(info.dealer))
          continue;

        if (districtsFilter.length && !districtsFilter.includes(info.district))
          continue;

        if (agentsFilter.length && !agentsFilter.includes(r.collectedBy))
          continue;

        const paymentDate = r.paymentDate ? new Date(r.paymentDate) : null;
        ...
        distObj.collectionAgents[r.collectedBy].totalCollection += amount;
      }
    }
    ...
    const hierarchy = Object.values(dealersMap).map((dealerObj) => ({
      ...
      districts: Object.values(dealerObj.districts).map((distObj) => ({
        ...
        collectionAgents: Object.values(distObj.collectionAgents),
      })),
    }));
    ...
    const agentPerformance = buildAgentPerformanceFromPayments(allPayments);
